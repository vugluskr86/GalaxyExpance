import { directoryEntries, executeSpectrumScan, hasDirectorySubscription, scanReadiness, scanSettings, signalSignature } from "../game/intel.js";
...
/** The scanner is a computer application rather than a map hotkey. The panel
 * exposes the actual radio controls; the canvas is intentionally a diagnostic
 * spectrum display, not a decorative progress bar. */
export class ScannerScene {
  ...
  analyze(){
    ...
    const result=executeSpectrumScan(this.sys.world,this.sys,this.target,this.settings,{computerId:this.computerId});
    this.message=result.ok?t("scan.result",{tier:result.record.tier,quality:Math.round(result.quality*100)}):t(`scan.error.${result.reason||"weak-signal"}`);
  ...
  draw(time){
    ...
    const ready=this.readiness();sctx.fillStyle=ready.ok?"#7ee08a":"#ff9a7d";sctx.font="9px 'Courier New',monospace";
    sctx.fillText((ready.ok?t("scan.ready"):t(`scan.error.${ready.reason||"weak-signal"}`))+` · ${t(`scanLabels.modes.${this.settings.mode}`)}`,32,262);
  }
  drawLabels(){}
  ...
  panelSpec(){
    const ready=this.readiness(),network=scannerClientConfig(this.sys.playerShip?.prop,this.computerId),scanners=networkAddressOptions(this.sys.playerShip?.prop,{kind:"scanner"}),antennas=networkAddressOptions(this.sys.playerShip?.prop,{kind:"antenna"});
    const networkStatus=ready.network?.ok?`${ready.network.computer.host} → ${ready.network.scanner.host}, ${ready.network.antenna.host}`:ready.reason;
    return [
      ...
      {kind:"action",label:t("scan.analyze"),run:()=>this.analyze()}
    ];
  }
}
